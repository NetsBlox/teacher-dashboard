<script lang="ts">
  ...
  import { GroupSettingsTable } from '$lib/data/tables/group_settings.svelte';
  ...
  let { serviceSettingsAR = $bindable(), groupId}: Props = $props();
  ...
  const tableParams: ConstructParams<SettingsTable, FlatServiceSetting> = {
    ...
    refresher: () => {
      serviceSettingsAR = getGroupSettings(fetch, groupId);
    },
  };
...
{:then settingsR}
  ...
  {:else}
    ...
    {@const table = new GroupSettingsTable({ ...tableParams, values: settings })}
